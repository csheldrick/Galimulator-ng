...
import type { ArtifactKind, GalaxyState, Id, SimSettings, SimEvent, EmpirePriority, SpyMission, ShipClass, ShipRole, WarFocus  } from "../types/sim";
...
export default function App() {
  ...
  const handleSetTranscendenceEnabled = useCallback((value: boolean) => { sim.setTranscendenceEnabled(value); refreshSnapshot(); }, [sim, refreshSnapshot]);
  ...
  const handleBuildShip = useCallback((sid: Id, shipClass: ShipClass, role?: ShipRole) => { withRefresh(() => sim.commandBuildShip(sid, shipClass, role)); }, [sim, withRefresh]);
  ...
  const handleDemandSubmission = useCallback((eid: Id) => { withRefresh(() => sim.commandDemandSubmission(eid)); }, [sim, withRefresh]);
  const handleSandboxBuildArtifact = useCallback((sid: Id, kind?: ArtifactKind) => { withRefresh(() => sim.sandboxBuildArtifact(sid, kind)); }, [sim, withRefresh]);
  const handleForceAlliance = useCallback((a: Id, b: Id) => { withRefresh(() => sim.forceAlliance(a, b)); }, [sim, withRefresh]);
  ...
  const empireControl = (
    <EmpireControlPanel
      ...
      onDemandSubmission={handleDemandSubmission}
    />
  );
    ...
    <div className="app-layout">
      <div className="play-area">
        ...
        <div className="bottom-hud tabbed">
          ...
          <div className="bottom-tab-body">
            {bottomTab === "control" ? (
              <ControlPanel snapshot={snapshot} selectedEmpireId={selectedEmpireId} followEmpireId={followEmpireId} running={running} onStart={handleStart} onPause={handlePause} onStep={handleStep} onRunTicks={handleRunTicks} onReset={handleReset} onNewSeed={handleNewSeed} onResetCamera={() => setResetCameraToken(t => t + 1)} onExportJson={handleExportJson} onExportReport={handleExportReport} onHeadlessReport={handleHeadlessReport} onPresetSweep={handlePresetSweep} onRiotGalaxy={() => withRefresh(() => sim.riotGalaxy())} onBalanceGalaxy={() => withRefresh(() => sim.balanceGalaxy())} onWebStarlanes={() => withRefresh(() => sim.sandboxWebStarlanes())} onGodEmpire={handleGodEmpire} transcendenceEnabled={snapshot.transcendenceEnabled ?? true} onSetTranscendenceEnabled={handleSetTranscendenceEnabled} onImportSave={handleImportSave} onSelectEmpire={handleSelectEmpire} onToggleFollow={handleToggleFollow} settings={settings} onSettingsChange={handleSettingsChange} viewOptions={viewOptions} onViewOptionsChange={setViewOptions} />
            ) : empireControl}
          </div>
        </div>
      </div>
      <div className="right-panel">
        <Section id="inspector" title="Inspector" collapsed={collapsedRight.inspector} onToggle={toggleRight}>
          <InspectorPanel snapshot={snapshot} selectedSystemId={selectedSystemId} selectedEmpireId={selectedEmpireId} selectedFleetId={selectedFleetId} followEmpireId={followEmpireId} onSelectEmpire={handleSelectEmpire} onSelectSystem={handleSelectSystem} onSelectFleet={handleSelectFleet} onClearSelection={handleClearSelection} onCancelFleet={handleCancelFleet} onToggleFollow={handleToggleFollow} onBoostSystem={id => withRefresh(() => sim.boostSystem(id))} onDevastateSystem={id => withRefresh(() => sim.devastateSystem(id))} onNeutralizeSystem={id => withRefresh(() => sim.neutralizeSystem(id))} onFoundEmpire={handleFoundEmpire} onBoostEmpire={id => withRefresh(() => sim.boostEmpire(id))} onStabilizeEmpire={id => withRefresh(() => sim.stabilizeEmpire(id))} onWeakenEmpire={id => withRefresh(() => sim.weakenEmpire(id))} onInflameEmpire={id => withRefresh(() => sim.inflameEmpire(id))} onPacifyEmpire={id => withRefresh(() => sim.pacifyEmpire(id))} onAdjustEmpire={(id, prop, dir) => withRefresh(() => sim.adjustEmpireProperty(id, prop, dir))} onSetEmpireMood={(id, mood) => withRefresh(() => sim.setEmpireMood(id, mood))} onSetEmpireIdeology={(id, ideology) => withRefresh(() => sim.setEmpireIdeology(id, ideology))} onToggleRulerTrait={(id, trait) => withRefresh(() => sim.toggleRulerTrait(id, trait))} onSetSystemTotem={(id, totem) => withRefresh(() => sim.setSystemTotem(id, totem))} onMoveSystem={(id, dx, dy) => withRefresh(() => sim.sandboxMoveSystem(id, dx, dy))} onConnectNearest={id => withRefresh(() => sim.sandboxConnectNearest(id))} onPruneLane={id => withRefresh(() => sim.sandboxPruneLongestLane(id))} onBuildArtifact={handleSandboxBuildArtifact} onBuildShip={(id, shipClass, role) => withRefresh(() => sim.buildShipAtSystem(id, shipClass, role))} onForceWar={(a, b) => withRefresh(() => sim.forceWar(a, b))} onForcePeace={(a, b) => withRefresh(() => sim.forcePeace(a, b))} onForceMerge={(a, b) => withRefresh(() => sim.forceMerge(a, b))} onForceAlliance={handleForceAlliance} onThrowMeteor={id => withRefresh(() => sim.sandboxThrowMeteor(id))} onSpawnMonster={() => withRefresh(() => sim.sandboxSpawnMonster())} onSpawnOddity={() => withRefresh(() => sim.sandboxSpawnOddity())} onSeedFaction={id => withRefresh(() => sim.sandboxSeedFaction(id))} />
        </Section>
