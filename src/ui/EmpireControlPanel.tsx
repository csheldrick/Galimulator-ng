import type { ArtifactKind, GalaxyState, Id, EmpirePriority, PlayerControlState, SpyMission, WarFocus } from "../types/sim";
...
export function EmpireControlPanel({
  ...
  onProposePeace, onProvokeWar, onSpyMission, onSponsorColonization, onAdoptReligion, onReformGovernment, onSetWarDirective,
}: Props) {
  ...
  return (
    <div className="empire-control-panel">
      ...
      <div className="cmd-grid">
        {ownsSys && (
          <>
            <CmdBtn label="Fortify" onClick={() => onFortify(selectedSystemId!)} cd={cooldownLeft("fortify", 20)} disabled={authority < 15} />
            <CmdBtn label="Stabilize" onClick={() => onStabilize(selectedSystemId!)} cd={cooldownLeft("stabilize", 10)} disabled={authority < 10} />
            <CmdBtn label="Build Artifact" onClick={() => onBuildArtifact(selectedSystemId!, ARTIFACT_KINDS[0])} cd={cooldownLeft("artifact", 120)} disabled={authority < 45 || controlled.wealth < 450 || !!selectedSys?.artifactId} />
          </>
        )}
        {selectedSys && (
          <CmdBtn label="Move Flagship" onClick={() => onMoveFlagship(selectedSystemId!)} cd={cooldownLeft("flagship", 8)} disabled={authority < 8} />
        )}
        {(isEnemy || ownsSys) && (
        ...
        {religions.length > 0 && (
          <CmdBtn label="Adopt Major Faith" onClick={() => onAdoptReligion(religions[0].id)} cd={cooldownLeft("religion", 90)} disabled={authority < 35 || controlled.stateReligionId === religions[0].id} />
        )}
        <CmdBtn label="Reform Gov" onClick={onReformGovernment} cd={cooldownLeft("reform", 140)} disabled={authority < 35 || corruption < 10} />
